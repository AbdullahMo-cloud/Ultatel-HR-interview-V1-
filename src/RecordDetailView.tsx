...
export default function RecordDetailView({
  ...
  const handleCopyAndEmail = async () => {
    ...
    try {
      ...
      alert(
        "A PDF report has been downloaded!\n\nAn email draft will now open. Please attach the downloaded PDF to the email.",
      );

      const subject = encodeURIComponent(
        `Candidate Evaluation: ${record.candidateName || "Unnamed"}`,
      );
      const to = userEmail ? encodeURIComponent(userEmail) : "";
      const cc = encodeURIComponent(
        "tarek.moaz@ultatel.com;Mohamed.AbouElHassan@ultatel.com;sam.casey@ultatel.com;sophia.brooks@ultatel.com;Mariem.Embaby@ultatel.com;nadine.miller@ultatel.com;mohamed.aboelqassem@ultatel.com;mennatullah.bahaa@ultatel.com;esra.alidrisi@ultatel.com;florence.mark@ultatel.com",
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <button
          onClick={onBack}
          className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-brand-blue transition-colors uppercase tracking-wider cursor-pointer"
        >
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 text-xs font-bold rounded shadow-sm hover:bg-slate-50 transition-all cursor-pointer"
          >
            <Pencil className="w-4.5 h-4.5 text-brand-blue" /> Edit Candidate
            Evaluation
          </button>
          <button
            onClick={handleCopyAndEmail}
            className="flex items-center gap-2 px-4 py-2 bg-brand-blue text-white text-xs font-bold rounded shadow hover:bg-brand-blue-light transition-all cursor-pointer"
          >
      ...
      <div
        ref={printRef}
        className="bg-white p-6 md:p-8 rounded shadow-sm border border-slate-200"
      >
        <div className="flex justify-between items-start border-b border-slate-100 pb-6 mb-6">
          ...
          <div className="text-right">
            ...
            <div
              className={`mt-2 inline-block px-3 py-1 text-[10px] font-bold rounded uppercase tracking-tighter ${scoreInfo?.color || "bg-slate-100 text-slate-500"}`}
            >
        ...
        {finalResult && (
          <div
            className={`mb-8 p-4 rounded-lg border flex items-start gap-4 shadow-sm ${resultColor}`}
          >
            <ResultIcon className="w-8 h-8 flex-shrink-0 mt-1" />
            <div>
              <div className="text-[10px] font-bold uppercase tracking-widest bg-white/50 inline-flex items-center px-2 py-0.5 rounded mb-2 border border-black/5">
                Final Rating
              </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
          {sections
            .filter((s) => s.questions.some((q) => q.type === "rating"))
            .map((section, idx) => {
              let maxPoints = 0;
              ...
              // Assign distinct icon and color based on section ID
              let SectionIcon = Activity;
              let iconColorClass = "text-brand-blue";

              if (section.id === "sec3") {
                SectionIcon = Brain;
                iconColorClass = "text-blue-500";
              } else if (section.id === "sec4") {
                SectionIcon = ShieldCheck;
                iconColorClass = "text-purple-500";
              } else if (section.id === "sec5") {
                SectionIcon = ClipboardList;
                iconColorClass = "text-amber-500";
              } else if (section.id === "sec6") {
                SectionIcon = Target;
                iconColorClass = "text-green-500";
              } else if (section.id === "sec7") {
                SectionIcon = MessageSquare;
                iconColorClass = "text-rose-500";
              } else if (section.id === "sec8") {
              ...
              return (
                <div
                  key={section.id}
                  className="p-4 bg-slate-50 rounded border border-slate-100 relative overflow-hidden group"
                >
                  <SectionIcon className="absolute -right-4 -bottom-4 w-16 h-16 text-slate-200/50 group-hover:scale-110 group-hover:text-slate-300 transition-all duration-300" />
                  <div className="text-[10px] uppercase font-bold text-slate-500 tracking-wider flex items-center gap-1.5">
                    <SectionIcon className={`w-4 h-4 ${iconColorClass} shrink-0`} />{" "}
                  ...
                  <div className="text-xl font-bold text-slate-900 mt-2 relative z-10">
                    {score} / {maxPoints}{" "}
                    <span className="text-sm text-slate-500 ml-1 font-semibold">
                      ({percent}%)
                    </span>
                  </div>
                </div>
        ...
        {(scoreInfo?.isMindsetFail ||
          ...
          (scoreInfo?.autoFails && scoreInfo.autoFails.length > 0)) && (
          <div className="mb-8 space-y-3">
            <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">
              Logged Flags & Warnings
            </h4>
            {scoreInfo?.isMindsetFail && (
            ...
            {scoreInfo?.autoFails?.map((failMsg: string, idx: number) => (
              <div
                key={idx}
                className="flex items-start p-3 bg-red-50 text-red-800 rounded-lg text-sm font-medium border border-red-100"
              >
        ...
        <div>
          ...
          <div className="space-y-8">
            {sections.map((sec, secIdx) => {
              ...
              const hasAnswers = sec.questions.some(
                (q) =>
                  record.answers[q.id] !== undefined &&
                  record.answers[q.id] !== "",
              );
              ...
              return (
                <div key={sec.id} className="space-y-4">
                  <h4
                    className={`text-sm font-black tracking-widest uppercase flex items-center gap-2 px-3 py-2 rounded-md ${theme.bg} ${theme.text} border ${theme.border}`}
                  >
                    <ClipboardList className={`w-4 h-4 ${theme.icon}`} />{" "}
                    {sec.title}
                  </h4>
                  <div className="bg-slate-50 rounded p-4 border border-slate-100 space-y-4">
                    {sec.questions.map((q) => {
                      ...
                      if (q.type === "rating" && q.options) {
                        const opt = (q.options as any[]).find(
                          (o) => o.points === rawAns,
                        );
                        if (opt)
                          displayAns = `${opt.points} pts - ${opt.label}: "${opt.text}"`;
                      }

                      const commentAns =
                        record.answers[`${q.id}_comment`] ||
                        record.answers[`${q.id}_other`];
                      if (commentAns) {
